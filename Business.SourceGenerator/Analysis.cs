...
{{
    ...
    static readonly Lazy<IDictionary<string, Func<GeneratorTypeArg, GeneratorTypeOpt, object>>> generatorType = new Lazy<IDictionary<string, Func<GeneratorTypeArg, GeneratorTypeOpt, object>>>(() => new Dictionary<string, Func<GeneratorTypeArg, GeneratorTypeOpt, object>> {{{0}}});

    public static IDictionary<string, Func<GeneratorTypeArg, GeneratorTypeOpt, object>> GeneratorType {{ get => generatorType.Value; }}

    static Func<GeneratorTypeArg, GeneratorTypeOpt, object> GetType(Type type)
    {{
        if (!GeneratorType.TryGetValue(Business.SourceGenerator.Utils.GetGenericType(type), out Func<GeneratorTypeArg, GeneratorTypeOpt, object> value))
        {{
            if (type.IsGenericType)
            {{
                var typeArgument = type.GetGenericArguments();
            
                var typeArgumentKey = 1 < typeArgument.Length ? $""<{{string.Join("", "", typeArgument.Select(c => Business.SourceGenerator.Utils.GetGenericType(c)))}}>"" : Business.SourceGenerator.Utils.GetGenericType(typeArgument[0]);

                if (!GeneratorType.TryGetValue(typeArgumentKey, out value))
                {{
                    return default;
                }}
            }}
    ...
    public System.Type MakeGenericType<Type>(System.Type type) => MakeGenericTypes(type, typeof(Type));

    public Type MakeGenericTypes(Type type, params Type[] typeArgument)
    {{
        if (type is null)
        {{
        ...
        var key = 1 < typeArgument.Length ? $""<{{string.Join("", "", typeArgument.Select(c => Business.SourceGenerator.Utils.GetGenericType(c)))}}>"" : Business.SourceGenerator.Utils.GetGenericType(typeArgument[0]);

        if (!GeneratorType.TryGetValue(key, out Func<GeneratorTypeArg, GeneratorTypeOpt, object> value))
        {{
            return default;
        }}

        return value(new GeneratorTypeArg(Business.SourceGenerator.Utils.GetGenericType(type.IsGenericTypeDefinition ? type : type.GetGenericTypeDefinition())), GeneratorTypeOpt.MakeGenericType) as Type;
    }}
    ...
    {{
        ...
        if (!(value(new GeneratorTypeArg(Business.SourceGenerator.Utils.GetGenericType(type.IsGenericType ? type.GetGenericTypeDefinition() : type)), GeneratorTypeOpt.Constructors) is IEnumerable<Constructor> constructors))
        {{
            return default;
        }}

        var constructor = Business.SourceGenerator.Utils.GetConstructor(constructors, args);
