...
namespace Business.SourceGenerator
{
    ...
    public sealed class GeneratorGenericType : Attribute
    {
        ...
        public static IReadOnlyDictionary<string, string> GetTypes(Expression.AnalysisInfoModel analysisInfo, TypeKeyFormat keyFormat = TypeKeyFormat.No)
        {
        ...
        static IEnumerable<ITypeSymbol> GetTypeReference(ConcurrentDictionary<string, SymbolInfo> typeSymbols, ITypeSymbol typeSymbol, params SyntaxKind[] kinds)
        {
    ...
    {
        IEnumerable<Type> GenericTypes { get; }
    ...
    {
        //const string SetGenericTypeSign = "Business.SourceGenerator.SetGenericType";

        public static AnalysisInfoModel AnalysisInfo = new AnalysisInfoModel(new ConcurrentDictionary<string, StringCollection>(), new ConcurrentDictionary<string, SymbolInfo>(), new ConcurrentDictionary<string, SymbolInfo>(), new ConcurrentDictionary<string, ConcurrentDictionary<string, AssignmentExpressionSyntax>>(), new ConcurrentDictionary<string, ConcurrentDictionary<string, SymbolInfo>>(), new ConcurrentDictionary<string, ConcurrentDictionary<string, SetGenericTypeArgs>>(), new ConcurrentDictionary<string, ConcurrentDictionary<string, ConcurrentDictionary<string, AttributeSyntax>>>());

        public readonly struct AnalysisInfoModel
        {
            internal AnalysisInfoModel(ConcurrentDictionary<string, StringCollection> syntaxTrees, ConcurrentDictionary<string, SymbolInfo> declaredSymbols, ConcurrentDictionary<string, SymbolInfo> typeSymbols, ConcurrentDictionary<string, ConcurrentDictionary<string, AssignmentExpressionSyntax>> staticAssignments, ConcurrentDictionary<string, ConcurrentDictionary<string, SymbolInfo>> invocations, ConcurrentDictionary<string, ConcurrentDictionary<string, SetGenericTypeArgs>> genericTypeSign, ConcurrentDictionary<string, ConcurrentDictionary<string, ConcurrentDictionary<string, AttributeSyntax>>> attributes)
            {
            ...
            //public StringCollection SyntaxTrees { get; }
        ...
        */
        ...
        public static void InitSymbols(SemanticModel model, SyntaxNode syntax)
        {
        ...
        */
        ...
        public static IEnumerable<SymbolInfo> GetGenericType(this SymbolInfo symbols, List<SymbolInfo> path = null, IEnumerable<ITypeSymbol> genericArguments = null)
        {
                ...
                case ParameterSyntax node:
                    {
                        ...
                        else if (AnalysisInfo.Invocations.TryGetValue(symbols.Source.GetFullNameOrig(), out ConcurrentDictionary<string, SymbolInfo> invocations))
                        {
            ...
            if (null != symbols.Symbol)
            {
                ...
                {
                    if ((symbols.Symbol is IFieldSymbol || symbols.Symbol is IPropertySymbol) && AnalysisInfo.StaticAssignments.TryGetValue(symbols.Symbol.GetFullName(), out ConcurrentDictionary<string, AssignmentExpressionSyntax> assignments))
                    {
        ...
        public static IEnumerable<SymbolInfo> GetMethodGenericParameters(string methodFullName, SyntaxNode syntaxNode = null, int? ordinal = null, ISymbol? other = null)
        {
            ...
            IEnumerable<SymbolInfo> nodes = null;

            if (null == syntaxNode)
            {
                if (AnalysisInfo.Invocations.TryGetValue(methodFullName, out ConcurrentDictionary<string, SymbolInfo> invocations))
                {
        ...
        public static SymbolInfo GetSymbolInfo(this ISymbol symbol)
        {
            if (symbol is null)
            {
            ...
            if (symbol is ITypeSymbol && AnalysisInfo.TypeSymbols.TryGetValue(key, out SymbolInfo type))
            {
            ...
            if (null != syntax && AnalysisInfo.DeclaredSymbols.TryGetValue(syntax.GetFullName(), out SymbolInfo info))
            {
                return info;
            }
        ...
        static SymbolInfo GetLastMethod(ISymbol source, List<SymbolInfo> path)
        {
        ...
        static Dictionary<IParameterSymbol, ExpressionSyntax> GetMethodParameters(SymbolInfo symbols)
        {
        ...
        static KeyValuePair<IParameterSymbol, ExpressionSyntax> GetMethodParameter(SymbolInfo symbols, int ordinal)
        {
            var parameters = GetMethodParameters(symbols);
        ...
        static ITypeSymbol GetTypeParameter(ISymbol source, List<SymbolInfo> path, ITypeSymbol symbol)
        {
        ...
        static IEnumerable<KeyValuePair<IParameterSymbol, ExpressionSyntax>> GetGenericParameter(IMethodSymbol method, SymbolInfo call, ISymbol? other)
        {
        ...
        static KeyValuePair<IParameterSymbol, ExpressionSyntax> GetGenericReturnParameter(IMethodSymbol method, SymbolInfo node)
        {
        ...
        ///// <returns></returns>
        //static SyntaxNode GetReferences(this ISymbol symbol)
        //{
        //    //symbol.GetSymbolInfo().References;
        //    //symbol.DeclaringSyntaxReferences
        //    if (null != syntax && DeclaredSymbols.TryGetValue(syntax.GetFullName(), out SymbolInfo info))
        //    {
        //        return info;
        //    }
        //    if (0 == symbol.DeclaringSyntaxReferences.Length && ProxyGenerator.semanticModels.TryGetValue(symbol.ContainingAssembly.Name, out Compilation compilation))
        //    {
        //        var symbols = compilation.GetSymbolsWithName(symbol.Name);

        //        if (symbols.Any())
        //        {
        //            var target = symbols.FirstOrDefault(c => c.GetFullName().EndsWith(symbol.GetFullName()));

        //            var syntax = target.DeclaringSyntaxReferences.FirstOrDefault()?.GetSyntax();

        //            if (null != syntax)
        //            {
        //                var model = compilation.GetSemanticModel(syntax.SyntaxTree);

        //                var sys = model.GetSymbols(syntax);
        //            }
        ...
        /*
        public static string GetFullName(this ISymbol symbol, bool dropArgs = false, bool isExplicitlyNamedTupleElement = true, bool dynamic2Object = false, bool noNullable = false)
        {
            if (symbol is null)
            {
                throw new ArgumentNullException(nameof(symbol));
            ...
            if (!string.IsNullOrEmpty(prefix))
            {
                ...
                {
                    nullable = noNullable ? string.Empty : "?";

                    if ("System.Nullable".Equals($"{prefix}.{symbol.Name}") && 0 < named?.TypeArguments.Length)
                    {
                        return $"{ GetFullName(named.TypeArguments[0], dropArgs, isExplicitlyNamedTupleElement, dynamic2Object, noNullable)}{nullable}";
                    }
                }
                ...
                if (null != named && 0 < named.TypeArguments.Length)
                {
                    if (named.IsTupleType)
                    {
                        ...
                        {
                            return $"({string.Join(", ", named.TupleElements.Select(c => (c.IsExplicitlyNamedTupleElement && isExplicitlyNamedTupleElement) ? $"{(TypeKind.Dynamic == c.Type.TypeKind && dynamic2Object ? "object" : GetFullName(c.Type, dropArgs, isExplicitlyNamedTupleElement, dynamic2Object, noNullable))} {c.Name}" : (TypeKind.Dynamic == c.Type.TypeKind && dynamic2Object ? "object" : GetFullName(c.Type, dropArgs, isExplicitlyNamedTupleElement, dynamic2Object, noNullable))))})";
                        }
                    }
                    else
                    {
                        args = $"<{string.Join(", ", named.TypeArguments.Select(c => (TypeKind.Dynamic == c.TypeKind && dynamic2Object ? "object" : GetFullName(c, dropArgs, isExplicitlyNamedTupleElement, dynamic2Object, noNullable))))}>";
                    }
                }
                ...
                if (symbol is IMethodSymbol method)
                {
                    ...
                    {
                        args = $"<{string.Join(", ", method.TypeArguments.Select(c => (TypeKind.Dynamic == c.TypeKind && dynamic2Object ? "object" : GetFullName(c, dropArgs, isExplicitlyNamedTupleElement, dynamic2Object, noNullable))))}>";
                    }

                    parameters = $"({string.Join(", ", method.Parameters.Select(c => (TypeKind.Dynamic == c.Type.TypeKind && dynamic2Object ? "object" : GetFullName(c.Type, dropArgs, isExplicitlyNamedTupleElement, dynamic2Object, noNullable))))})";
                }
            ...
            {
                return string.Join(", ", anonymousType.Constructors.Select(c => $"({string.Join(", ", c.Parameters.Select(c2 => $"{(TypeKind.Dynamic == c2.Type.TypeKind && dynamic2Object ? "object" : GetFullName(c2.Type, dropArgs, isExplicitlyNamedTupleElement, dynamic2Object, noNullable))} {c2.Name}"))})"));
            }
            ...
            if (symbol is IArrayTypeSymbol array)
            {
                ...
                return $"{GetFullName(array.ElementType, dropArgs, isExplicitlyNamedTupleElement, dynamic2Object)}{rank}";
            }
        ...
        */
        ...
        /*
        public static IEnumerable<ITypeSymbol> GetTypeReference2(ITypeSymbol typeSymbol, params SyntaxKind[] kinds)
        {
            if (typeSymbol is null)
            {
                throw new ArgumentNullException(nameof(typeSymbol));
            ...
            foreach (var item in AnalysisInfo.TypeSymbols.Values)
            {
                ...
                {
                    case TypeKind.Class:
                        if (null != typeSymbol2.BaseType && typeSymbol2.BaseType.Equals(typeSymbol))
                        {
                            list.Add(typeSymbol2);

                            list.AddRange(GetTypeReference(typeSymbol2));
                        }
                        break;
                    case TypeKind.Interface:

                        var typeSymbol3 = typeSymbol2;

                        if (null == typeSymbol3.BaseType)
                        {
                            typeSymbol3 = typeSymbol3.OriginalDefinition;
                        }

                        if (typeSymbol3.AllInterfaces.Any(c => typeSymbol.OriginalDefinition.Equals(c.OriginalDefinition, SymbolEqualityComparer.Default)))
                        {
                            list.Add(typeSymbol2);
                            //Set(declared, typeSymbol, methodGenericPosition, genericPosition);
                            list.AddRange(GetTypeReference(typeSymbol2));
                        }
        ...
        public static IEnumerable<ITypeSymbol> GetTypeReference(ITypeSymbol typeSymbol, params SyntaxKind[] kinds)
        {
            if (typeSymbol is null)
            {
                throw new ArgumentNullException(nameof(typeSymbol));
        ...
        //static readonly IEqualityComparer<ISymbol> SymbolEquality = Equality<ISymbol>.CreateComparer(c => c.GetFullName());
        static readonly IEqualityComparer<ITypeSymbol> TypeSymbolEquality = Equality<ITypeSymbol>.CreateComparer(c => c.GetFullName());
        */
        #endregion
        ...
        public static (FileScopedNamespaceDeclarationSyntax Declaration, UsingDirectiveSyntax[] Usings) MakeGenericTypeAll(this TypeParameterSyntax syntaxParameter, BaseTypeDeclarationSyntax replace, string name = null)
        {
            ...
            if (AnalysisInfo.DeclaredSymbols.TryGetValue(replace.GetFullName(), out SymbolInfo replaceInfo))
            {
            ...
            if (AnalysisInfo.DeclaredSymbols.TryGetValue(declarationSyntax.GetFullName(), out SymbolInfo origInfo))
            {
        ...
        /// <param name="name"></param>
        /// <returns></returns>
        public static (FileScopedNamespaceDeclarationSyntax Declaration, UsingDirectiveSyntax[] Usings) MakeGenericTypeAll(this TypeParameterSyntax syntaxParameter, SymbolInfo replace, string name = null)
        {
            if (syntaxParameter is null)
            {
                throw new ArgumentNullException(nameof(syntaxParameter));
        ...
        */
        ...
        public static IEnumerable<SyntaxToken> GetTokens(this TypeParameterSyntax syntaxParameter)
        {
            ...
            if (!AnalysisInfo.DeclaredSymbols.TryGetValue(syntaxParameter.GetFullName(), out SymbolInfo info))
            {
                throw new ArgumentNullException(nameof(syntaxParameter));
            }
            ...
            IEnumerable<SyntaxToken> ClassOrStruct(TypeDeclarationSyntax node)
            {
                ...
                {
                    if (AnalysisInfo.DeclaredSymbols.TryGetValue(item.Parent.GetFullName(), out SymbolInfo info2))
                    {
        ...
        public static IEnumerable<SyntaxNode> GetNodes(this TypeParameterSyntax syntaxParameter)
        {
            ...
            if (!AnalysisInfo.DeclaredSymbols.TryGetValue(syntaxParameter.GetFullName(), out SymbolInfo targetInfo))
            {
                throw new ArgumentNullException(nameof(syntaxParameter));
            }
            ...
            IEnumerable<SyntaxNode> FindNodes(SyntaxNode node)
            {
                ...
                foreach (var item in node.DescendantTokens())
                {
                    ...
                    if (AnalysisInfo.DeclaredSymbols.TryGetValue(parent.GetFullName(), out SymbolInfo itemInfo))
                    {
    ...
    public class Generator : ISourceGenerator
    {
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            var types = GeneratorGenericType.GetTypes(Expression.AnalysisInfo, keyFormat);
            ...
            if (types.Any())
            {
                var field = SyntaxFactoryExt.ParseField("genericTypes", typeof(Type[]), SyntaxKind.ReadOnlyKeyword).Initializer(SyntaxFactoryExt.ArrayInitializerExpression(types.Select(c => SyntaxFactory.TypeOfExpression(SyntaxFactory.ParseTypeName(c.Value))).ToArray()));

                var property = SyntaxFactoryExt.ParseProperty("GenericTypes", typeof(IEnumerable<Type>), SyntaxFactory.AccessorDeclaration(SyntaxKind.GetAccessorDeclaration).WithSemicolonToken(SyntaxFactory.Token(SyntaxKind.SemicolonToken)).WithExpressionBody(SyntaxFactory.ArrowExpressionClause(SyntaxFactory.IdentifierName("genericTypes"))), null, SyntaxKind.PublicKeyword);
                ...
                field,
                property,
                #region public static Type MakeGenericType(string key)

                SyntaxFactoryExt.ParseMethod("MakeGenericType").AddModifiers(SyntaxKind.PublicKeyword)
                ...
                .WithBody(
                    ...
                    SyntaxFactory.LocalDeclarationStatement(SyntaxFactoryExt.VariableDeclaration("var", SyntaxFactoryExt.VariableDeclarator("typeName", SyntaxFactoryExt.InvocationExpression(SyntaxFactoryExt.MemberAccessExpression(SyntaxFactoryExt.MemberAccessExpression("TypeNameFormatter", "TypeName"), "GetFormattedName"), typeName, SyntaxFactory.BinaryExpression(SyntaxKind.BitwiseOrExpression, SyntaxFactory.BinaryExpression(SyntaxKind.BitwiseOrExpression, SyntaxFactory.BinaryExpression(SyntaxKind.BitwiseOrExpression, SyntaxFactoryExt.MemberAccessExpression(SyntaxFactoryExt.MemberAccessExpression("TypeNameFormatter", "TypeNameFormatOptions"), "Namespaces"), SyntaxFactoryExt.MemberAccessExpression(SyntaxFactoryExt.MemberAccessExpression("TypeNameFormatter", "TypeNameFormatOptions"), "NoKeywords")), SyntaxFactoryExt.MemberAccessExpression(SyntaxFactoryExt.MemberAccessExpression("TypeNameFormatter", "TypeNameFormatOptions"), "NoAnonymousTypes")), SyntaxFactoryExt.MemberAccessExpression(SyntaxFactoryExt.MemberAccessExpression("TypeNameFormatter", "TypeNameFormatOptions"), "NoGeneric")))))),

                    SyntaxFactory.LocalDeclarationStatement(SyntaxFactoryExt.VariableDeclaration("var", SyntaxFactoryExt.VariableDeclarator("args", SyntaxFactoryExt.InvocationExpression(SyntaxFactoryExt.MemberAccessExpression("string", "Join"), SyntaxFactoryExt.ParseLiteral(", "), SyntaxFactoryExt.InvocationExpression(SyntaxFactoryExt.MemberAccessExpression("typeArguments", "Select"), SyntaxFactory.SimpleLambdaExpression(SyntaxFactory.Parameter(SyntaxFactory.ParseToken("c")), SyntaxFactoryExt.InvocationExpression(SyntaxFactoryExt.MemberAccessExpression(SyntaxFactoryExt.MemberAccessExpression("TypeNameFormatter", "TypeName"), "GetFormattedName"), SyntaxFactory.IdentifierName("c"), SyntaxFactory.BinaryExpression(SyntaxKind.BitwiseOrExpression, SyntaxFactory.BinaryExpression(SyntaxKind.BitwiseOrExpression, SyntaxFactoryExt.MemberAccessExpression(SyntaxFactoryExt.MemberAccessExpression("TypeNameFormatter", "TypeNameFormatOptions"), "Namespaces"), SyntaxFactoryExt.MemberAccessExpression(SyntaxFactoryExt.MemberAccessExpression("TypeNameFormatter", "TypeNameFormatOptions"), "NoKeywords")), SyntaxFactoryExt.MemberAccessExpression(SyntaxFactoryExt.MemberAccessExpression("TypeNameFormatter", "TypeNameFormatOptions"), "NoAnonymousTypes"))))))))),

                    SyntaxFactory.IfStatement(SyntaxFactoryExt.InvocationExpression(SyntaxFactoryExt.MemberAccessExpression(SyntaxFactory.PredefinedType(SyntaxFactory.Token(SyntaxKind.StringKeyword)), SyntaxFactory.IdentifierName("IsNullOrEmpty")), SyntaxFactory.IdentifierName("args")), SyntaxFactory.Block(SyntaxFactory.ReturnStatement(typeName))),

                    SyntaxFactory.ReturnStatement(SyntaxFactoryExt.InvocationExpression(SyntaxFactory.IdentifierName("MakeGenericType"), SyntaxFactory.InterpolatedStringExpression(SyntaxFactory.Token(SyntaxKind.InterpolatedStringStartToken), new SyntaxList<InterpolatedStringContentSyntax>()
        ...
        public void Execute2(GeneratorExecutionContext context)
        {
            ...
            var code2 = System.IO.File.ReadAllText(System.IO.Path.Combine(@"D:\Repos\Business.SourceGenerator-main\Business.SourceGenerator\Assets\src", "code.txt"));
            ...
            var types = GeneratorGenericType.GetTypes(Expression.AnalysisInfo);
            ...
            var sds = Expression.AnalysisInfo.DeclaredSymbols;
            var sds2 = Expression.AnalysisInfo.Invocations;
            //var sds3 = Expression.SemanticModels;
            var sds4 = Expression.AnalysisInfo.StaticAssignments;
            var sds5 = Expression.AnalysisInfo.TypeSymbols;
            ...
            if (0 < Expression.AnalysisInfo.GenericTypeSign.Count)
            {
                foreach (var item in Expression.AnalysisInfo.GenericTypeSign)
                {
            ...
            foreach (var item in Expression.AnalysisInfo.TypeSymbols)
            {
            ...
            var ds = Expression.AnalysisInfo.DeclaredSymbols.Where(c => c.Value.Declared is IFieldSymbol || c.Value.Declared is IPropertySymbol);
            ...
            var classCode = $"public class class{Expression.AnalysisInfo.DeclaredSymbols.Count} {{ {ss2} }}";
            var code = $"namespace codes{Expression.AnalysisInfo.DeclaredSymbols.Count} {{ {classCode} }}";
